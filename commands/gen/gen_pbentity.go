...
func generateEntityMessageDefinition(name string, fieldMap map[string]*gdb.TableField, req *generatePbEntityReq) string {
	...
	for index, name := range names {
		array = append(array, generateMessageFieldForPbEntityInjectTag(fieldMap[name], req))
		array = append(array, generateMessageFieldForPbEntity(index+1, fieldMap[name], req))
	}
...
func generateMessageFieldForPbEntityInjectTag(field *gdb.TableField, req *generatePbEntityReq) []string {
	ormTag := field.Name
	...
	return []string{
		"    #",
		fmt.Sprintf(" #//@inject_tag: orm:\"%s\"", ormTag),
	}
}
